...
BOOL ConfigureBoard(HANDLE boardHandle, int rate, int range);
BOOL AcquireData(HANDLE boardHandle, int length, const char * filename, uint32_t channelMask);

void usage (const char * arg[])
{
    cerr << "USAGE: " << arg[0] << " <sampling rate (in MSPS)> <voltage range> <capture length (in seconds)> <filename> [num_channels -- 1 or 2]\n\n"
            "    Valid sampling rates: 180, 160, 125, 100, 50, 20, or 10\n"
            "    Valid ranges: 200mV,400mV,800mv, or 2V\n\n"
            "    For filename, please indicate filename only (no path, especially no absolute path).\n\n"
            "    File will be saved under /Digitizer/captured-data/\n\n"
            "    Example of command to capture 1min at 100MSPS, 800mV range: " << arg[0] << " 100 800mV 60 test-file.bin\n\n"; 
}
...
int main (int argc, const char * arg[])
{
    ...
    const int rate = atoi(arg[1]);
    const int range = atoi(arg[2]);
    const int length = atoi(arg[3]);

    const int channels = ((argc == 6) ? atoi(arg[5]) : 1);
    ...
    if (!ConfigureBoard(boardHandle, rate, range))
    {
    ...
    char out[100];
    strftime (out, sizeof(out), "%Y-%m-%d-%H%M", tt);

    ostringstream filename;
    filename << "/Digitizer/captured-data/" << out << "--" << rate << "MSPS--" << rng << "--" << arg[4];

    uint32_t channelMask = CHANNEL_A;
    if (channels == 2)
    {
        channelMask |= CHANNEL_B;
    }
    // Make an acquisition, saving sample data to file

    if (!AcquireData(boardHandle, length, filename.str().c_str(), channelMask))
    {
...
BOOL ConfigureBoard(HANDLE boardHandle, int rate, int range)
{
    ...
    samplesPerSec = rate * 1e6;
    unsigned int rate_code = SAMPLE_RATE_100MSPS;
    switch (rate)
    {
        case 20:
            rate_code = SAMPLE_RATE_20MSPS;
            break;
        case 50:
            rate_code = SAMPLE_RATE_50MSPS;
            break;
        case 10:
            rate_code = SAMPLE_RATE_10MSPS;
            break;
        case 100:
            rate_code = SAMPLE_RATE_100MSPS;
            break;
        case 125:
            rate_code = SAMPLE_RATE_125MSPS;
            break;
        case 160:
            rate_code = SAMPLE_RATE_160MSPS;
            break;
        case 180:
            rate_code = SAMPLE_RATE_180MSPS;
            break;
        default:
            cout << "WARNING: INVALID SAMPLING RATE --- DEFAULTING TO 100MSPS" << endl;
            rate_code = SAMPLE_RATE_100MSPS;
            break;
    }

    unsigned int range_code = INPUT_RANGE_PM_200_MV;
    switch (range)
    {
        case 200:
            range_code = INPUT_RANGE_PM_200_MV;
            break;
        case 400:
            range_code = INPUT_RANGE_PM_400_MV;
            break;
        case 800:
            range_code = INPUT_RANGE_PM_800_MV;
            break;
        case 2:
            range_code = INPUT_RANGE_PM_2_V;
            break;
    }
    ...
    retCode = AlazarSetTriggerOperation(boardHandle,
                                        ...
                                        130, // U32 -- trigger level from 0 (-range) to 255 (+range)
                                        TRIG_ENGINE_K,
                                        TRIG_DISABLE,
                                        TRIGGER_SLOPE_POSITIVE,
                                        128);
...
void print_time()
{
    ...
    struct tm * tt = localtime(&t);
    char out[100];
    strftime (out, sizeof(out), "%Y-%m-%d-%H:%M:%S", tt);
    ostringstream decimals;
    decimals << (tv.tv_usec / 1000000.0);

    cout << "Capture started at: " << out << decimals.str().substr(1) << endl; 

    cout << tv.tv_sec << decimals.str().substr(1) << endl;
}
...
BOOL AcquireData(HANDLE boardHandle, int length, const char * filename, uint32_t channelMask)
{
    ...
    double acquisitionLength_sec = length;

    // TODO: Select the number of samples in each DMA buffer
    U32 samplesPerBuffer = 2048000;
        ...
        while (buffersCompleted < buffersPerAcquisition)
        {
            ...
            if (success)
            {
                ...
                if (saveData)
                {
                    ...
                    //uint16_t downsampled[(bytesPerBuffer/2)/50];
                    //for (int k = 0; k < (bytesPerBuffer/2)/50; k++){
                    //    downsampled[k] = pBuffer[k*50];
                        //if (k % 2){fprintf(stderr, "k, downsampled[k] %u, %u\n", k, downsampled[k]);}
                    //}

                    //size_t bytesWritten = fwrite(pBuffer, sizeof(BYTE), bytesPerBuffer/50, fpData);
